...
!
!  mobbrmsd_DORGLQ generates an \( M \)-by-\( N \) real matrix \( Q \)
...
!
!  Reference DORGLQ is provided by [netlib](http://www.netlib.org/lapack/explore-html/).
!
...
!
  INFO = 0
  NB = mobbrmsd_ILAENV(1, 'DORGLQ', ' ', M, N, K, -1)
  LWKOPT = MAX(1, M) * NB
  WORK(1) = LWKOPT
  LQUERY = (LWORK == -1)
  if (M < 0) then
...
!
    NX = MAX(0, mobbrmsd_ILAENV(3, 'DORGLQ', ' ', M, N, K, -1))
    if (NX < K) then
!
...
!
        NB = LWORK / LDWORK
        NBMIN = MAX(2, mobbrmsd_ILAENV(2, 'DORGLQ', ' ', M, N, K, -1))
      end if
    end if
  end if
!
