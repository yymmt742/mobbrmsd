...
!
  interface node
    module procedure node_new_as_root
  end interface node
!
contains
!
...
!
    if (dmat%l < 1) then
      ...
      allocate (res%per(dmat%m(1)%g))
    end if
!
  contains
!
    pure function active_index(m) result(res)
      type(d_matrix), intent(in) :: m(:)
      integer(IK)                :: i, res
      res = 0
      do i = 1, SIZE(m)
        if (m(i)%g < 1) cycle
...
!
! pure function node_new(dmat, depth, iprm, isym, per, H, C, W) result(res)
!   type(d_matrix), intent(in) :: dmat
