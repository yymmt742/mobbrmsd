...
!
  subroutine test1(fail)
    ...
    type(partial_rmsd)     :: pr, pr2, pr3
    real(RK)               :: X(d * n), Y(d * n)
!
    X = [sample(d, n)]
    Y = [MATMUL(SO3(), RESHAPE(X, [d, n]))]
    !Y = [MATMUL(MATMUL(SO3(), RESHAPE(X, [d, n])), PER15())]
!
    pr = partial_rmsd(d, 5, X(:15), Y(:15))
    print*,pr%sd(), sd(d, 5, X, Y)
    pr2 = pr%append(5, X(16), Y(16))
    print*,pr2%sd(), sd(d, 10, X, Y)
    pr3 = pr2%append(5, X(31), Y(31))
    print*,pr3%sd(), sd(d, n, X, Y)
!
