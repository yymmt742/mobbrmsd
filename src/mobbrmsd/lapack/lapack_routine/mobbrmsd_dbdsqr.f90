...
!
!  mobbrmsd_DBDSQR computes the singular values and,
...
!
    if (NRU > 0) &
&      call mobbrmsd_DLASR('R', 'V', 'F', NRU, N, WORK(1), WORK(N), &
&                  U, LDU)
    if (NCC > 0) &
&      call mobbrmsd_DLASR('L', 'V', 'F', N, NCC, WORK(1), WORK(N), &
&                  C, LDC)
  end if
!
...
!
    call mobbrmsd_DLASV2(D(M - 1), E(M - 1), D(M), SIGMN, SIGMX, SINR, &
&                        COSR, SINL, COSL)
    D(M - 1) = SIGMX
    E(M - 1) = ZERO
    D(M) = SIGMN
...
!
    if (NCVT > 0) &
&      call mobbrmsd_DROT(NCVT, VT(M - 1, 1), LDVT, VT(M, 1), LDVT, &
&                 COSR, SINR)
    if (NRU > 0) &
&      call mobbrmsd_DROT(NRU, U(1, M - 1), 1, U(1, M), 1, COSL, &
&                 SINL)
    if (NCC > 0) &
&      call mobbrmsd_DROT(NCC, C(M - 1, 1), LDC, C(M, 1), LDC, COSL, &
&                 SINL)
    M = M - 2
    GO TO 60
  end if
...
&                             WORK(N), VT(LL, 1), LDVT)
      if (NRU > 0) &
&         call mobbrmsd_DLASR('R', 'V', 'F', NRU, M - LL + 1, &
&                             WORK(NM12 + 1), &
&                             WORK(NM13 + 1), U(1, LL), LDU)
      if (NCC > 0) &
&         call mobbrmsd_DLASR('L', 'V', 'F', M - LL + 1, NCC, &
&                             WORK(NM12 + 1), &
...
!
      if (NCVT > 0) &
&         call mobbrmsd_DLASR('L', 'V', 'B', M - LL + 1, NCVT, &
&                             WORK(NM12 + 1), &
...
!
      ...
      D(N + 1 - I) = SMIN
      if (NCVT > 0) &
&         call mobbrmsd_DSWAP(NCVT, VT(ISUB, 1), LDVT, VT(N + 1 - I, 1), LDVT)
      if (NRU > 0) &
&         call mobbrmsd_DSWAP(NRU, U(1, ISUB), 1, U(1, N + 1 - I), 1)
      if (NCC > 0) &
&         call mobbrmsd_DSWAP(NCC, C(ISUB, 1), LDC, C(N + 1 - I, 1), LDC)
    end if
  end do
  GO TO 220
!
