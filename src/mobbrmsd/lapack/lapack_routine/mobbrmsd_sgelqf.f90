...
!   and tau in TAU(i).
!
!  Reference SGELQF is provided by [netlib](http://www.netlib.org/lapack/).
!
...
!
  INFO = 0
  NB = mobbrmsd_ILAENV(1, 'SGELQF', ' ', M, N, -1, -1)
  LWKOPT = M * NB
  WORK(1) = LWKOPT
  LQUERY = (LWORK == -1)
  if (M < 0) then
...
!
  ...
  if (NB > 1 .and. NB < K) then
    ...
    !
    NX = MAX(0, mobbrmsd_ILAENV(3, 'SGELQF', ' ', M, N, -1, -1))
    if (NX < K) then
      !
      ...
      if (LWORK < IWS) then
        ...
        !
        NB = LWORK / LDWORK
        NBMIN = MAX(2, mobbrmsd_ILAENV(2, 'SGELQF', ' ', M, N, -1, -1))
      end if
    end if
  end if
!
