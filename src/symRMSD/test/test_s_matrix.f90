...
!
    ...
    print*,memsize_s_matrix(s)
    print*,worksize_s_matrix(s)
!
    c(1)%p = 1
    c(1)%w = c(1)%p + memsize_c_matrix(c(1))
    s(1)%p = c(1)%w
    s(1)%w = s(1)%p + memsize_s_matrix(s(1))
    c(2)%p = s(1)%w
    c(2)%w = c(2)%p + memsize_c_matrix(c(2))
    s(2)%p = c(2)%w
    s(2)%w = s(2)%p + memsize_s_matrix(s(2))
    c(3)%p = s(2)%w
    c(3)%w = c(3)%p + memsize_c_matrix(c(3))
    s(3)%p = c(3)%w
    s(3)%w = s(3)%p + memsize_s_matrix(s(3))
!
    allocate (W(SUM(memsize_c_matrix(c) + worksize_c_matrix(c) &
   &              + memsize_s_matrix(s) + worksize_s_matrix(s))))
!
    W(:) = 999
    call c_matrix_eval(c(1), b(1), ms(1), X, Y, W, W)
    call s_matrix_eval(s(1), b(1), W(c(1)%p), W, W)
    call c_matrix_eval(c(2), b(2), ms(2), X, Y, W, W)
    call s_matrix_eval(s(2), b(2), W(c(2)%p), W, W)
    call c_matrix_eval(c(3), b(3), ms(3), X, Y, W, W)
    call s_matrix_eval(s(3), b(3), W(c(3)%p), W, W)
!
    print'(3f9.3)', W(s(1)%p:s(1)%p + memsize_s_matrix(s(1)) - 1)
    print'(4f9.3)', W(s(2)%p:s(2)%p + memsize_s_matrix(s(2)) - 1)
    print'(5f9.3)', W(s(3)%p:s(3)%p + memsize_s_matrix(s(3)) - 1)
!
